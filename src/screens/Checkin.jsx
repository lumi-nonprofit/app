...
export function CheckinStep1({ draft, setDraft, onNext }) {
  return (
    <Screen>
      ...
      <View accessibilityRole="radiogroup" accessibilityLabel="Jak se právě teď cítíš?" style={styles.quadGrid}>
        {MOODS.map((m) => {
      ...
      <Card style={styles.intensityCard}>
        ...
        <Slider
          ...
          style={styles.slider}
          onValueChange={(v) => setDraft({ intensity: v })}
        />
      ...
      <LumiCTA disabled={!draft.mood} onPress={onNext} hint="Nejdřív klepni na stav, který je teď nejblíž.">
        Pokračovat
      </LumiCTA>
    </Screen>
  );
...
export function CheckinStep2({ draft, setDraft, onBack, onSave }) {
  ...
  return (
    <Screen>
      ...
      <LumiCTA disabled={draft.words.length === 0} onPress={onSave} hint="Vyber aspoň jedno slovo — pomáhá v Přehledech.">
        Uložit zápis
      </LumiCTA>
    </Screen>
  );
...
/* ---------- potvrzení ---------- */
const CONFIRM_TIPS = {
  napeti: { icon: "wind", tint: colors.accentSoft, color: palette.sun700, title: "Dech 4-7-8", subtitle: "Pomáhá při napětí", badge: ["accent", "3 min"] },
  utlum: { icon: "moon", tint: palette.lilac100, color: palette.lilac700, title: "Klidné usínání", subtitle: "Zvuky a audio na dobrou noc", badge: ["lilac", "večer"] },
  energie: { icon: "notebook-pen", tint: colors.positiveSoft, color: palette.sage700, title: "Zápis do deníku", subtitle: "Zachyť, co se dnes povedlo", badge: null },
  klid: { icon: "footprints", tint: colors.positiveSoft, color: palette.sage700, title: "Všímavá procházka", subtitle: "Meditace v pohybu · venku", badge: ["positive", "venku"] },
};
  ...
  return (
    <Screen contentStyle={{ justifyContent: "center" }}>
      <View style={styles.confirmHero}>
        ...
        <View>
          <Text accessibilityRole="header" style={styles.confirmTitle}>Uloženo.</Text>
...
const styles = StyleSheet.create({
  ...
  quadName: { ...font.display(600), fontSize: 16, lineHeight: leading.body(16), color: colors.textStrong },
  quadAxis: { ...font.body(400), fontSize: 12, lineHeight: leading.body(12), color: palette.ink700, marginTop: 1 },

  /* krok 1 — intenzita */
  intensityCard: { paddingTop: 16, paddingHorizontal: 20, paddingBottom: 12 },
  intensityHead: { flexDirection: "row", justifyContent: "space-between", alignItems: "baseline", marginBottom: 2 },
  intensityLabel: { ...font.body(600), fontSize: 14.5, lineHeight: leading.body(14.5), color: colors.textBody },
  intensityValue: { ...font.body(600), fontSize: 13.5, lineHeight: leading.body(13.5), color: palette.sun700 },
  slider: { height: 44 },
  intensityScale: { flexDirection: "row", justifyContent: "space-between" },
  intensityScaleText: { ...font.body(400), fontSize: 12.5, lineHeight: leading.body(12.5), color: palette.ink700 },
  ...
  moodSummaryName: { ...font.body(600), fontSize: 14.5, lineHeight: leading.body(14.5), color: colors.textStrong },
  moodSummaryIntensity: { ...font.body(400), fontSize: 13, lineHeight: leading.body(13), color: palette.ink700 },
  wordsHint: { ...font.body(400), fontSize: 13.5, lineHeight: leading.body(13.5), color: palette.ink700, marginBottom: 8 },
  tagsLabel: { ...font.body(600), fontSize: 14.5, lineHeight: leading.body(14.5), color: colors.textBody, marginTop: 4, marginBottom: 8 },
  ...
  confirmBody: { ...font.body(400), fontSize: 16, lineHeight: leading.body(16), color: colors.textBody, textAlign: "center" },
