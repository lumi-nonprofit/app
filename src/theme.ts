...
const DISPLAY_FAMILIES: Record<FontWeight, string> = {
...
const BODY_FAMILIES: Record<FontWeight, string> = {
...
export const font = {
  display: (weight: FontWeight = 400): { fontFamily: string } => ({
    fontFamily: DISPLAY_FAMILIES[weight],
  }),
  body: (weight: FontWeight = 400): { fontFamily: string } => ({
    fontFamily: BODY_FAMILIES[weight],
  }),
};

/* unitless line-height / em tracking → absolutní body podle velikosti písma */
export const leading = {
  tight: (size: number): number => Math.round(size * 1.15),
  snug: (size: number): number => Math.round(size * 1.3),
  body: (size: number): number => Math.round(size * 1.55),
};
export const tracking = {
  display: (size: number): number => -0.02 * size,
  label: (size: number): number => 0.06 * size,
};

/* výchozí text — náhrada dědičnosti z `body` v CSS (RN Text nedědí) */
export const text: { base: TextStyle } = {
  base: {
    ...font.body(400),
    fontSize: type.base,
...
/* --- motion --- */
export const dur = { fast: 150, base: 240, slow: 420, breath: 4000 } as const;
